//$Header: /home/deegree/jail/deegreerepository/deegree/src/org/deegree/io/datastore/sde/SDEInsertHandler.java,v 1.8 2006/09/26 16:44:51 mschneider Exp $
/*----------------    FILE HEADER  ------------------------------------------
...
import org.deegree.io.datastore.sql.transaction.FeatureRow;
import org.deegree.io.datastore.sql.transaction.InsertRow;
...
/**
 * Handler for <code>Insert</code> operations (usually contained in <code>Transaction</code>
 * requests).
 *
...
public class SDEInsertHandler extends AbstractSDERequestHandler {
    ...
    public List<FeatureId> performInsert( List<Feature> features )
        ...
        for ( int i = 0; i < features.size(); i++ ) {
            ...
            if ( feature.getId().startsWith( FeatureIdAssigner.EXIST_PREFIX ) ) {
                String msg = "feature already exists " + feature.getName() + " id="
            ...
            FeatureId fid = new FeatureId(
                                           ft.getGMLId(),
                                           new Object[] { FeatureId.removeFIDPrefix(
                                                                                     feature.getId(),
                                                                                     ft.getGMLId() ) } );
            fids.add( fid );
        }
        ...
        if ( cycle != null ) {
            ...
            String msg = "ERROR_FK_CYCLE " + sb.toString();
            throw new TransactionException( msg );
        }
    ...
    private FeatureRow insertFeature( Feature feature )
        ...
        if ( fidFields.length > 1 ) {
            throw new TransactionException(
                                            "Insertion of features with compound feature ids is not "
                                                                    + "supported." );
        }
        ...
        FeatureId fid = new FeatureId( ft.getGMLId(), new Object[] { fidValue } );
        ...
        if ( relations == null || relations.length == 0 ) {
            ...
            if ( !relations[0].isFromFK() ) {
                ...
                for ( int i = 0; i < pkFields.length; i++ ) {
                    ...
                    if ( pkField == null ) {
                        String msg = "Missing foreign key " + pkFields[i].getField() + " / "
                                     + pkFields[i].getTable();
                        throw new TransactionException( msg );
                    }
        ...
        if ( relations.length == 1 ) {
            ...
            if ( relations[0].isFromFK() ) {
                ...
                for ( int i = 0; i < pkFields.length; i++ ) {
                    ...
                    if ( pkField == null ) {
                        String msg = "Missing foreign key " + pkField.getColumnName() + " / "
                                     + pkField.getTable();
                        throw new TransactionException( msg );
                    }
                    int pkColumnType = pkField.getSQLType();
                ...
                if ( !cutLink ) {
                    ...
                    featureRow.setColumn( fkFields[0].getField(), pkField.getValue(),
                                          pkField.getSQLType(), false );
                }
            }
    ...
    private boolean doesFeatureExist( FeatureRow featureRow )
        ...
        SeQuery stmt = null;
        try {
            stmt = buildFeatureSelect( pkField.getColumnName(), pkField.getValue(),
                                       featureRow.getTable() );
            stmt.execute();
    ...
    private SeQuery buildFeatureSelect( String fidColumn, Object fidValue, String table ) {
        SeQuery query = null;
        try {
            SeSqlConstruct constr = new SeSqlConstruct( table, fidColumn + "='"
                                                               + fidValue.toString() + "'" );
    ...
    private SeInsert createStatement( InsertRow row )
        ...
        inserter.setState( conn.getState().getId(), new SeObjectId( SeState.SE_NULL_STATE_ID ),
                           SeState.SE_STATE_DIFF_NOCHECK );
        Collection fields = row.getColumns();
        ...
        for ( i = 0; i < columns.length; i++ ) {
            ...
            SDEAdapter.setRowValue( insertRow, i, field.getValue(),
                                    SDEAdapter.mapSQL2SDE( field.getSQLType() ) );
        }
        return inserter;
    }
 ...
 * $Log: SDEInsertHandler.java,v $
 * Revision 1.8  2006/09/26 16:44:51  mschneider
 * Refactored because of moving of TransactionException.
 *
 * Revision 1.7  2006/08/29 15:53:57  mschneider
