...
/**
 * <p>A class for reading from and writing to DBase index files (*.ndx), maybe not
 * 100% xbase compatible!</p>
 * 
 * <p>The fileformat is described at http://www.e-bachmann.dk/computing/databases/xbase/index.html</p>
 * 
 * <p>This index is suitable for indexing both unique and non-unique columns.
 * Unique indexing is much faster than non-unique because it use a faster algorithm.</p>
 * 
 * <p>The index file is a B+tree (sometimes called a paged B-tree) that consist of pages. There are
 * two page types, leaves and non-leafs. The starting page (eg. the page the search
 * algorithm starts) is the root page.</p>
 * 
 * <p><b>Searching goes as follows:</b>
 * <ul>
 * <li>load the root page (eg. the starting page)</li>
 * <li>if the page is a leaf<ul>
 * <li>search for the requested key</li></ul></li>
 * <li>if the page is not a leaf (eg. the page has subpages)<ul>
 * <li>search for a key that is equal to or bigger than the requested key</li>
 * <li>load the lower page</li>
 * <li>continue searching inside the lower page</li></ul></li>
 * </ul></p>
 * 
 * <p>Above algorithm is implemented in two different methods, one for
 * unique indexes and one for non-unique indexes. Searching unique indexes
 * is easier because the algorithm is finished as soon as it has found a key,
 * the non-unique version of the algorithm has to find all keys present in the index.<p>
 * 
 * <p><b>Inserting goes as follows:</b>
 * <ul>
 * <li>find the leaf page the key has to insert in</li>
 * <li>insert the key in the leaf page</li>
 * <li>if the leaf page is full (eg. validEntries > noOfKeysPerPage)<ul>
 * <li>split the leaf page (results in two leaf pages)</li>
 * <li>add the first item of the new page to the parent page</li></ul></li>
 * <li>if the parent page is also full<ul>
 * <li>split the parent page<li>
 * <li>add the first item of the new page to it's parent page</li>
 * <li>etc.</li></ul></li>
 * </ul></p>
 * 
 * <p>If a page that splits does not have a parent page then a new page is created. This page is the new
 * starting page</p>
 * 
 * <p>Handling different data types:
 * The index can handle strings and numbers. Numbers are always stored als IEEE doubles. The method
 * addKey checks the given key and throws an exception if the datatype of the key doesn't suit the index</p>
 * 
...
public class DBaseIndex {
    ...
    private class Page {
         ...
         */
        int search( Comparable key, Stack searchStack )
                                throws IOException {
         ...
         */
        ArrayList searchDup( Comparable key )
                                throws IOException {
         ...
         */
        int searchDupPos( Comparable key, Stack searchStack )
                                throws IOException {
            if ( validEntries == 0 ) //Page is empty
                return number;
    ...
    private DBaseIndex( String name, int startingPageNo, int numberOfPages, int sizeOfKeyRecord,
                       int keyLength, int noOfKeysPerPage, int keyType, boolean uniqueFlag,
    ...
    public static DBaseIndex createIndex( String name, String column, int keyLength,
                                         boolean uniqueFlag, boolean numbers )
                            throws IOException {
    ...
    public void addKey( Comparable key, int record )
                            throws IOException, KeyAlreadyExistException, InvalidKeyTypeException,
                            KeyTooLongException {
        if ( key == null )
            throw new NullPointerException();
...
/* ********************************************************************
 ...
 $Log: DBaseIndex.java,v $
 Revision 1.5  2006/08/06 20:54:33  poth
 code formating

 Revision 1.4  2006/05/29 16:15:42  poth
 code simplification
