...
public class MapInfoDataSource {
    ...
    private void setUpFactories() throws Exception {
        ...
        for (int i = 0; i < hColumnsNames.size(); i++) {
            ...
            PropertyType ftp = FeatureFactory.createSimplePropertyType(
                    new QualifiedName((String) hColumnsNames.get(i)), typeClass, true);
            colAttribs.add(ftp);
        }

        PropertyType ftp = FeatureFactory.createSimplePropertyType(
                new QualifiedName("GEOM"), Types.GEOMETRY, true);
        ...
        try {
            PropertyType[] ftps = (PropertyType[]) colAttribs
                    .toArray(new PropertyType[colAttribs.size()]);
            pointType = FeatureFactory.createFeatureType(filename.toString()
                    + "_point", false, ftps);
        } catch (Exception schexp) {
            throw new Exception("SchemaException setting up point factory : ",
                    schexp);
        }

        // Set up Line factory
        // Add default attribute type
        ftp = FeatureFactory.createSimplePropertyType(new QualifiedName("GEOM"),
                                                       Types.GEOMETRY, true);
        ...
        try {
            PropertyType[] ftps = (PropertyType[]) colAttribs
                    .toArray(new PropertyType[colAttribs.size()]);
            lineType = FeatureFactory.createFeatureType(filename.toString()
                    + "_line", false, ftps);
        } catch (Exception schexp) {
            throw new Exception("SchemaException setting up line factory : ",
                    schexp);
        }
        ...
        try {
            PropertyType[] ftps = (PropertyType[]) colAttribs
                    .toArray(new PropertyType[colAttribs.size()]);
            polygonType = FeatureFactory.createFeatureType(filename.toString()
                    + "_poly", false, ftps);
        } catch (Exception schexp) {
            throw new Exception(
                    "SchemaException setting up polygon factory : ", schexp);
        }
    }
        ...
        try {
            while ((readMifLine(mifReader) != null)
                ...
                if (clause(line).equalsIgnoreCase(CLAUSE_DELIMETER)) {
                    ...
                    hDelimeter = line.replace('\"', ' ').trim().substring(
                            line.trim().indexOf(' ')).trim();
            ...
            throw new Exception("IOException reading MIF header : "
                    + ioexp.getMessage());
        }
    }
    ...
    private Feature readRegionObject(BufferedReader mifReader,
        ...
        try {
            ...
            for (int i = 0; i < numpolygons; i++) {
                ...
                try {
                    Polygon pol = geomFactory.createPolygon(geomFactory
                            .createLinearRing((Coordinate[]) coords
                                    .toArray(new Coordinate[coords.size()])),
                            null);
            ...
            Geometry polyGeom = geomFactory
                    .createMultiPolygon((Polygon[]) polys
    ...
    private ArrayList readMid(BufferedReader midReader) throws Exception {
        ...
        if (midReader == null) { return new ArrayList(); }
        ...
        while (quotes.hasMoreTokens()) {
            StringTokenizer delimeters = new StringTokenizer(
                    quotes.nextToken(), hDelimeter + "\0");
