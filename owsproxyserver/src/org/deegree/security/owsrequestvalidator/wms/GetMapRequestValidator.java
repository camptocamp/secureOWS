...
public class GetMapRequestValidator extends AbstractWMSRequestValidator {
	...
	private GeoTransformer wgs84Transformer = null;
	...
	public GetMapRequestValidator(Policy policy) {
		super(policy);
		try {
			wgs84Transformer = new GeoTransformer("EPSG:4326");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * validates the incomming GetMap request against the policy assigend to a
	 * validator
	 * 
	 * @param request
	 *            request to validate
	 * @param user
	 *            name of the user who likes to perform the request (can be
	 *            null)
	 */
	public void validateRequest(OGCWebServiceRequest request, User user)
		...
		validateBBOX(condition, env, wmsreq.getSrs());
		validateLayers(condition, wmsreq.getLayers());
		validateBGColor(condition, ColorUtils.toHexCode("0x", wmsreq
				.getBGColor()));
	...
	/**
	 * checks if the passed envelope is valid against the maximum bounding box
	 * defined in the policy. If <tt>user</ff> != <tt>null</tt> the 
	 ...
	 */
	private void validateBBOX(Condition condition, Envelope envelope, String requestSRS)
			throws InvalidParameterValueException {
		...
		Envelope env = GeometryFactory.createEnvelope(Double.parseDouble(d[0]),
				Double.parseDouble(d[1]), Double.parseDouble(d[2]), Double
						.parseDouble(d[3]), null);

		String conditionSRS = "EPSG:4326";
		...
		Envelope tranformedEnvelope = envelope;
		...
		if (requestSRS != null) {
			try {
				...
				// or improve CS Factory
				e.printStackTrace();
			}
		}
		boolean denied = denyCompletelyOutside ? !env.intersects(tranformedEnvelope)
				: !env.contains(tranformedEnvelope);

		if (denied) {
			if (!op.isUserCoupled()) {
				// if not user coupled the validation has failed
				throw new InvalidParameterValueException(INVALIDBBOX
						+ op.getFirstAsString());
			}
			userCoupled = true;
	...
	/**
	 * checks if the passed layres/styles are valid against the layers/styles
	 * list defined in the policy. If <tt>user</ff> != <tt>null</tt> the 
	...
	private void validateLayers(Condition condition, GetMap.Layer[] layers)
		...
		for (int i = 0; i < layers.length; i++) {
			...
			if (vs == null) {
				if (!op.isUserCoupled()) {
					throw new InvalidParameterValueException(INVALIDLAYER
							+ layers[i].getName());
				}
	...
	/**
	 * checks if the passed bgcolor is valid against the bgcolor(s) defined in
	 * the policy. If
	...
	/**
	 * checks if the passed transparency is valid against the transparency
	...
	/**
	 * checks if the requested map area/size is valid against the minimum
	 * resolution defined in the policy. If
	...
	private void validateResolution(Condition condition, GetMap gmr)
		...
		try {
			...
			throw new InvalidParameterValueException(StringTools
		...
		if (scale < compareRes) {
			if (!op.isUserCoupled()) {
				throw new InvalidParameterValueException(INVALIDRESOLUTION
						+ scale);
			}
			userCoupled = true;
	...
	/**
	 * checks if the passed reference to a SLD document is valid against the
	...
	private void validateSLD(Condition condition, URL sldRef)
		...
		try {
			...
			String s = org.deegree.i18n.Messages.getMessage(
					"WMS_SLD_IS_NOT_VALID", sldRef);
			...
			for (int i = 0; i < nl.length; i++) {
				...
				if (vs == null) {
					...
					userCoupled = true;
						...
						&& vs.indexOf("$any$") < 0 && vs.indexOf(style) < 0) {
					// a style is valid for a layer if it's the default style
					// or the layer accepts any style or a style is explicit
					// defined
					// to be valid
					if (!op.isUserCoupled()) {
						throw new InvalidParameterValueException(INVALIDSTYLE
	...
	/**
	 * checks if the passed user is allowed to perform a GetMap request
	 * containing a SLD_BODY parameter.
	 * 
	 * @param condition
	 ...
	 * @throws InvalidParameterValueException
	 */
	private void validateSLD_Body(Condition condition,
			StyledLayerDescriptor sld_body)
			throws InvalidParameterValueException {

		/*
		 * 
		 * the problem is that sld_body never is null because it always will
		 * contain the requested layers and styles
		 * 
		 * OperationParameter op = condition.getOperationParameter( SLD_BODY ); //
		 * version is valid because no restrictions are made if ( sld_body ==
		 * null ||op.isAny() ) return; // at the moment it is just evaluated if
		 * the user is allowed // to perform a SLD request or not. no content
		 * validation will // be made boolean isAllowed = false; if (
		 * op.isUserCoupled() ) { //TODO // get comparator list from security
		 * registry } if (!isAllowed ) { throw new
		 * InvalidParameterValueException( INVALIDSLD_BODY ); }
		 */
	}
	...
	private void validateAgainstRightsDB(GetMap wmsreq, User user)
		...
		if (user == null) {
			...
			throw new UnauthorizedException(Messages.format(
					"RequestValidator.NOACCESS", sb));
		}
		...
		fps[0] = FeatureFactory.createFeatureProperty("version", wmsreq
				.getVersion());
		fps[1] = FeatureFactory.createFeatureProperty("width", new Integer(
				wmsreq.getWidth()));
		fps[2] = FeatureFactory.createFeatureProperty("height", new Integer(
				wmsreq.getHeight()));
		Envelope env = wmsreq.getBoundingBox();
		try {
			env = wgs84Transformer.transform(env, wmsreq.getSrs());
		} catch (Exception e) {
			throw new InvalidParameterValueException(
					"A:condition envelope isn't in " + "the right CRS ", e);
		}
		Object geom = null;
		try {
		...
		fps[4] = FeatureFactory.createFeatureProperty("format", wmsreq
				.getFormat());
		fps[5] = FeatureFactory.createFeatureProperty("bgcolor", ColorUtils
				.toHexCode("0x", wmsreq.getBGColor()));
		fps[6] = FeatureFactory.createFeatureProperty("transparent", ""
				+ wmsreq.getTransparency());
		fps[7] = FeatureFactory.createFeatureProperty("exceptions", wmsreq
				.getExceptions());
		fps[8] = FeatureFactory.createFeatureProperty("resolution", scale);
		fps[9] = FeatureFactory.createFeatureProperty("sld", wmsreq
				.getSLD_URL());

		GetMap.Layer[] layers = wmsreq.getLayers();
		for (int i = 0; i < layers.length; i++) {
			fps[10] = FeatureFactory.createFeatureProperty("style", layers[i]
					.getStyleName());
	...
	private double calcScale(GetMap request) throws Exception {
		...
		return MapUtils.calcScale(request.getWidth(), request.getHeight(),
				bbox, crs, DEFAULT_PIXEL_SIZE);
		/*
		 * if ( !request.getSrs().equalsIgnoreCase( "EPSG:4326" ) ) { //
		 * transform the bounding box of the request to EPSG:4326 bbox =
		 * gt.transformEnvelope( bbox, request.getSrs()); }
		 * 
		 * double dx = bbox.getWidth() / request.getWidth(); double dy =
		 * bbox.getHeight() / request.getHeight();
		 *  // create a box on the central map pixel to determine its size in
		 * meter Position min = GeometryFactory.createPosition(
		 * bbox.getMin().getX() + dx * ( request.getWidth() / 2d - 1d ),
		 * bbox.getMin().getY() + dy * ( request.getHeight() / 2d - 1d ) );
		 * Position max = GeometryFactory.createPosition( bbox.getMin().getX() +
		 * dx * ( request.getWidth() / 2d ), bbox.getMin().getY() + dy * (
		 * request.getHeight() / 2d ) );
		 * 
		 * double sc = calcDistance( min.getY(), min.getX(), max.getY(),
		 * max.getX() );
		 * 
		 * return sc;
		 */
	...
	private static FeatureType createFeatureType() {
		...
		ftps[0] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"version"), Types.VARCHAR, false);
		ftps[1] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"width"), Types.INTEGER, false);
		ftps[2] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"height"), Types.INTEGER, false);
		ftps[3] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"GEOM"), Types.GEOMETRY, false);
		ftps[4] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"format"), Types.VARCHAR, false);
		ftps[5] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"bgcolor"), Types.VARCHAR, false);
		ftps[6] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"transparent"), Types.VARCHAR, false);
		ftps[7] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"exceptions"), Types.VARCHAR, false);
		ftps[8] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"resolution"), Types.DOUBLE, false);
		ftps[9] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"sld"), Types.VARCHAR, false);
		ftps[10] = FeatureFactory.createSimplePropertyType(new QualifiedName(
				"style"), Types.VARCHAR, false);
 ...
 * GetMapRequestValidator.java,v $ Revision 1.9 2006/11/27 09:07:52 poth JNI
 * integration of proj4 has been removed. The CRS functionality now will be done
 * by native deegree code.
 * 
 * Revision 1.8 2006/11/07 09:56:11 poth support for GetMap SLD parameter added
 * 
 * Revision 1.7 2006/10/17 20:31:19 poth ** empty log message ***
 * 
 * Revision 1.6 2006/09/27 16:46:41 poth transformation method signature changed
 * 
 * Revision 1.5 2006/09/25 12:47:00 poth bug fixes - map scale calculation
 * 
 * Revision 1.4 2006/08/10 07:17:52 poth bug fix - removing Arrays.asList calls
 * for transforming op.geValues because accoring to refactoring this method it
 * already returns a list
 * 
 * Revision 1.3 2006/08/02 14:16:16 poth message for throwing an
 * UnauthrizedException corrected
 * 
 * Revision 1.2 2006/08/02 09:45:09 poth changes required as consequence of
 * changing OperationParameter
 * 
 * Revision 1.1 2006/07/23 08:44:53 poth refactoring - moved validators assigned
 * to OWS into specialized packages
 * 
 * Revision 1.27 2006/06/07 12:19:38 poth ** empty log message ***
 * 
 * Revision 1.26 2006/06/05 09:59:25 poth method for conversation from
 * java.awt.Color to its Hex code representation centralized in
 * framework.ColorUtil
 * 
 * Revision 1.25 2006/05/25 09:53:31 poth adapated to changed/simplified policy
 * xml-schema
 * 
 * 
 ******************************************************************************/