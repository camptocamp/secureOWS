//$Header: /home/deegree/jail/deegreerepository/deegree/src/org/deegree/model/coverage/grid/ByteGridCoverage.java,v 1.5 2006/10/12 15:43:14 poth Exp $
/*----------------    FILE HEADER  ------------------------------------------
 ...
 *
 * @version 1.0. $Revision: 1.5 $, $Date: 2006/10/12 15:43:14 $
 *
 * @since 2.0
 */
public class ByteGridCoverage extends AbstractGridCoverage {
     ...
     */
    public ByteGridCoverage(CoverageOffering coverageOffering, Envelope envelope,
                            byte[][][] data) {
    ...
    public byte[] getDataBlock(GridRange gridRange, byte[] destination)
        ...
        if ( data != null ) {
            ...
            if ( destination == null ) {
                destination = new byte[ (hi[0] - lo[0]) * (hi[1] - lo[1]) ];
            }
    ...
    public short[] getDataBlock(GridRange gridRange, short[] destination)
        ...
        data = getDataBlock( gridRange, data );
        if ( destination == null ) {
            destination = new short[ data.length ];
        }
        for (int i = 0; i < destination.length; i++) {
            destination[i] = data[i];
        }
    ...
    public BufferedImage getAsImage(int xAxis, int yAxis) {
        ...
        if ( data != null ) {
            ...
            SampleModel sm = 
            	new BandedSampleModel(DataBuffer.TYPE_BYTE, data[0][0].length,
    ...
    private BufferedImage createBufferedImage(int xAxis, int yAxis) {
    	...
    	switch (sampleDim) {
    		case 1: return new BufferedImage( xAxis, yAxis, BufferedImage.TYPE_BYTE_GRAY);
    		case 3: return new BufferedImage( xAxis, yAxis, BufferedImage.TYPE_INT_RGB);
    		default: return new BufferedImage( xAxis, yAxis, BufferedImage.TYPE_INT_ARGB);
    	}
    }
...
$Log: ByteGridCoverage.java,v $
Revision 1.5  2006/10/12 15:43:14  poth
comment corrected
...
Revision 1.1.1.1  2005/01/05 10:35:57  poth
no message
