 ...
 * 
 * @version 2.0, $Revision: 1.18 $, $Date: 2006/07/12 16:59:32 $
 * 
 * @since 2.0
 */
public class CatalogueConfigurationDocument extends CatalogueCapabilitiesDocument {
    ...
    public CatalogueConfiguration getConfiguration() throws InvalidConfigurationException {
        ...
        try {
            ...
            Element filterCapabilitiesElement = 
                (Element) XMLTools.getNode( getRootElement(), "ogc:Filter_Capabilities", 
                                            nsContext );
            if ( filterCapabilitiesElement != null ) {
                filterCapabilities = new FilterCapabilities100Fragment( filterCapabilitiesElement,
            ...
            configuration = new CatalogueConfiguration( parseVersion(), parseUpdateSequence(),
                ...
                filterCapabilities, getDeegreeParams(), getSystemId() );
    ...
    public CatalogueDeegreeParams getDeegreeParams() throws InvalidConfigurationException {
        ...
        try {
            ...
            OnlineResource defaultOnlineResource = parseOnLineResource( XMLTools
                .getRequiredChildElement( "DefaultOnlineResource", DEEGREECSW, element ) );
            ...
            int requestTimeLimit = XMLTools.getNodeAsInt( element, "./deegreecsw:RequestTimeLimit",
                nsContext, 2 );

            // 'deegreecsw:Encoding'-element (optional, default: UTF-8)
            String characterSet = 
                XMLTools.getStringValue( "Encoding", DEEGREECSW, element, "UTF-8" );
            ...
            SimpleLink wfsResource = parseSimpleLink( XMLTools.getRequiredChildElement(
                "WFSResource", DEEGREECSW, element ) );

            // 'deegreecsw:CatalogAddresses'-element (optional)
            Element catalogAddressesElement = XMLTools.getChildElement( "CatalogAddresses",
                DEEGREECSW, element );
            OnlineResource[] catalogAddresses = new OnlineResource[0];
            if ( catalogAddressesElement != null ) {
                // 'deegreecsw:CatalogAddresses'-element (optional)
                ElementList el = XMLTools.getChildElements( "CatalogAddress", DEEGREECSW,
                    catalogAddressesElement );
            ...
            Element elem = 
                (Element)XMLTools.getNode( element, "deegreecsw:TransactionInputXSLT", nsc );
            if ( elem != null ) {
                transInXslt = parseOnLineResource( elem );    
            }
            OnlineResource transOutXslt = null;
            elem = 
                (Element)XMLTools.getNode( element, "deegreecsw:TransactionOutputXSLT", nsc );
            if ( elem != null ) {
                transOutXslt = parseOnLineResource( elem );
            }
            if ( ( transInXslt != null && transOutXslt == null ) || 
                 ( transInXslt == null && transOutXslt != null ) ) {
                throw new InvalidConfigurationException ( "if CSW-deegreeParam " +
            ...
            Element harvestRepositoryElement = XMLTools.getChildElement( "HarvestRepository",
                DEEGREECSW, element );
            ...
            if ( harvestRepositoryElement != null ) {
                ...
                if ( connectionElement != null ) {
                    harvestRepository = new JDBCConnection(
            ...
            deegreeParams = new CatalogueDeegreeParams( defaultOnlineResource,
                cacheSize, requestTimeLimit, characterSet, wfsResource, catalogAddresses,
                harvestRepository, defaultOutputSchema, transInXslt, transOutXslt );
        } catch (XMLParsingException e) {
    ...
    /**
     * Overwritten to cope with additional deegree CSW specific attributes (used in
     * the "outputSchema" parameter element).
     * 
    ...
    protected OWSDomainType getOWSDomainType( String operation, Element parameterElement ) 
        ...
        if ( "GetRecords".equals( operation ) && "outputSchema".equals( parameterName ) ) {
            // "ows:Value"-elements
            NodeList valueNodes = 
                parameterElement.getElementsByTagNameNS( OWSNS.toString(), "Value" );
            CatalogueOutputSchemaValue[] values = 
                new CatalogueOutputSchemaValue[valueNodes.getLength()];
            for (int i = 0; i < valueNodes.getLength(); i++) {
                String value = XMLTools.getStringValue( valueNodes.item( i ) );
                String input = 
                    XMLTools.getRequiredAttrValue( "input", DEEGREECSW, valueNodes.item( i ) );
                String output = 
                    XMLTools.getRequiredAttrValue( "output", DEEGREECSW, valueNodes.item( i ) );
            ...
            for (int i = 0; i < valueNodes.getLength(); i++) {
                String value = XMLTools.getStringValue( valueNodes.item( i ) );
                String schema = 
                    XMLTools.getRequiredAttrValue( "schema", DEEGREECSW, valueNodes.item( i ) );
...
$Log: CatalogueConfigurationDocument.java,v $
Revision 1.18  2006/07/12 16:59:32  poth
required adaptions according to renaming of OnLineResource to OnlineResource

Revision 1.17  2006/07/12 14:46:16  poth
