//$Header: /home/deegree/jail/deegreerepository/deegree/src/org/deegree/owscommon_new/XMLFactory.java,v 1.2 2006/08/24 06:43:04 poth Exp $
/*----------------    FILE HEADER  ------------------------------------------
...
public class XMLFactory extends org.deegree.ogcbase.XMLFactory {
    ...
    public static void appendBaseCapabilities( Element root, OWSCommonCapabilities caps ) {
        ...
        if( identification != null ) appendServiceIdentification( root, identification );
        
        ServiceProvider provider = caps.getServiceProvider();
        if( provider != null ) appendServiceProvider( root, provider );
        
        OperationsMetadata metadata = caps.getOperationsMetadata();
        if( metadata != null ) appendOperationsMetadata( root, metadata );
        
        root.setAttribute( "version", caps.getVersion() );
        
        String updateSequence = caps.getUpdateSequence();
        if( updateSequence != null ) root.setAttribute( "updateSequence", updateSequence );
    }
    ...
    public static void appendOperation( Element root, Operation operation ) {
        ...
        elem.setAttribute( "name", operation.getName().getAsString() );
    }
    ...
    public static void appendDomainType( Element root, String tagName, DomainType data ) {
        ...
        elem.setAttribute( "name", data.getName().getAsString() );
    }
